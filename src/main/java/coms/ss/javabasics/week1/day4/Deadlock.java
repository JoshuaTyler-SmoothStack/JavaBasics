...
public class Deadlock {
  ...
  public static void main(String[] args) {
    ...
    int lockObj1 = 1;
    int lockObj2 = 2;
    ControlledThread thread1 = deadLock.createDeadlockThread(lockObj1, lockObj2, "1");
    ControlledThread thread2 = deadLock.createDeadlockThread(lockObj2, lockObj1, "2");
    thread1.run();
    thread2.run();
    ...
    KitUtils.setTimeout(() -> {
      if (thread1.isRunning() && thread2.isRunning()) {
        System.out.println("Both threads detected running after 5 seconds. \nDisabling threads . . .");
        thread1.interrupt();
        thread2.interrupt();
        System.exit(0);
      }
   ...
   */
  public ControlledThread createDeadlockThread(Object lockObj1, Object lockObj2, String threadName) {
    Runnable onExecute = () -> {
      try {
    ...
    ControlledThread thread = new ControlledThread(onExecute, onInterrupt, threadName, 200);
    return thread;
  }
}